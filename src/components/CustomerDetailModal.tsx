    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" onClick={closeDetailModal}>
            <div className="card-base w-full max-w-4xl max-h-[90vh] flex flex-col modal-content" onClick={e => e.stopPropagation()}>
                {/* Header */}
                <div className="p-4 border-b border-border-light dark:border-border-dark flex justify-between items-center flex-shrink-0">
                    <div className="flex items-center">
                        <img className="h-12 w-12 rounded-full object-cover" src={customer.avatar} alt={customer.name} />
                        <div className="ml-4">
                            <h3 className="text-xl font-semibold">{customer.name}</h3>
                            <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark">{customer.district}, {customer.state}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-4">
                        <button onClick={handleDelete} className="text-sm text-red-500 hover:underline">Delete Customer</button>
                        <button onClick={closeDetailModal} className="text-text-secondary-light dark:text-text-secondary-dark hover:text-red-500 text-xl">
                    ...
                    <div className="flex space-x-1 p-1 overflow-x-auto">
                        {TABS.map(tab => (
                            <button
                                ...
                                className={`py-2 px-4 font-medium text-sm rounded-md transition-colors duration-200 flex-shrink-0 ${activeTab === tab.id ? 'bg-black/5 dark:bg-white/10 text-text-primary-light dark:text-text-primary-dark' : 'text-text-secondary-light dark:text-text-secondary-dark hover:bg-black/5 dark:hover:bg-white/5'}`}
                            >
