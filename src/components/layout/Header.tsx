...
const Header: React.FC = () => {
    ...
    const { openAddCustomerModal, openBulkImportModal, openCommandPalette } = useApp();
        ...
        <header className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg border-b border-border-light dark:border-border-dark p-3 flex justify-between items-center sticky top-0 z-40">
            <div className="flex items-center gap-4">
                ...
                <div className="flex items-center bg-gray-100 dark:bg-black/20 p-1 rounded-lg">
                    <button onClick={() => navigate('/')} className={`px-3 py-1 text-sm font-semibold rounded-md transition-all duration-200 ${getButtonClass('/')}`}>
                        <i className="fas fa-tachometer-alt mr-2 opacity-80"></i>Dashboard
                    </button>
                    <button onClick={() => navigate('/analytics')} className={`px-3 py-1 text-sm font-semibold rounded-md transition-all duration-200 ${getButtonClass('/analytics')}`}>
            ...
            <div className="flex items-center gap-3">
                ...
                <button
                    ...
                    className="w-9 h-9 flex items-center justify-center text-red-500 bg-red-50 dark:bg-red-900/20 rounded-full hover:bg-red-100 dark:hover:bg-red-900/40"
                    title="Sign Out"
                >
